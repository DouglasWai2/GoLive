...
public class ScreenCaptureController extends AbstractVideoCaptureController {
    ...
    private final Runnable projectionStoppedCallback;

    public ScreenCaptureController(Context context, int width, int height,
            Intent mediaProjectionPermissionResultData, float resolutionScale,
            Runnable projectionStoppedCallback) {
        super((int)(width * resolutionScale), (int)(height * resolutionScale), DEFAULT_FPS);

        this.mediaProjectionPermissionResultData = mediaProjectionPermissionResultData;

        this.context = context;
        this.projectionStoppedCallback = projectionStoppedCallback;
        ...
        VideoCapturer videoCapturer =
                new ScreenCapturerAndroid(mediaProjectionPermissionResultData, new MediaProjection.Callback() {
                    ...
                    public void onStop() {
                        ...
                        orientationListener.disable();
                        if (projectionStoppedCallback != null) {
                            projectionStoppedCallback.run();
                        }
                        MediaProjectionService.abort(context);
                        stopCapture();
